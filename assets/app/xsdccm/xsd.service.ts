...
import xsdccmCfg from './../../../config/xsdccm.json';
...
export class XsdService {
  ...
  constructor(
    ...
    this.xsdcfg=xsdccmCfg
    this.projname=xsdccmCfg.project
    this.configurl=xsdccmCfg.configurl
    this.title=xsdccmCfg.title
    this.headertxt=xsdccmCfg.headertxt
    this.configResources(this.configurl)
  }

  configResources(configlink: string) {
    this.http.get(configlink).subscribe(
      (response) => {
        var configlist = JSON.parse(response["_body"])
        //console.log(configlist)
        this.projname = configlist.project
        //console.log("projname: " + this.projname)
        this.Configs[this.projname] = configlist
        this.Configs.push(configlist)
        //console.log("xmlproject: " + configlist[this.projname].title)
        var resrcs = configlist.resources
        this.Resources[this.projname] = []
        for (var r in resrcs) {
          this.Resources[this.projname][resrcs[r].name] = configlist.host + configlist.project + '/file/' + resrcs[r].name
        }
        var projimp = configlist.implementations
        for (var ip in projimp) {
          projimp[ip]
          this.configResources(projimp[ip].srcurl) 
        }
      })
  }
  ...
  validateXml(xmlstrng: string, xsdname: string) {
    ...
    var valdata = {
      package: this.selectedxsd,
      xmlstr: xmlstrng,
      xsdname: xsdname,
    };
    return this.http
      .post(this.xsdcfg.host.concat("validate"), valdata)
      .pipe(
        map((response: Response) => {
  ...
  verifyStr(cfg: any, name: string, str: string) {
    ...
    return this.http
      .post(cfg["host"].concat("verify"), { id: name, digest: digest })
      .pipe(
        map((response) => {
          var vresp = JSON.parse(response["_body"]);
  ...
  getComponents(xsdjsondata: object) {
    ...
    this.xsd[this.selectedxsd] = new XsdSchema();
    this.xsd[this.selectedxsd].complextypes = this.complextypes[
      this.selectedxsd
    ];
  ...
  downloadDocument(url: string, name: string, callback) {
    ...
    return this.http
      .get(url, { headers: headers, responseType: ResponseContentType.Text })
      .map((response: Response) => {
